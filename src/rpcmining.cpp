...
//    Litecoin and modified for HanaCoin.
//
//    Parameters: lookup (int) - How many blocks to look into the past.
//    Returns: Estimated HanaCoin network hash rate (uint64_t)
//
...
// WM - Implementation of getnetworkhashps for HanaCoin
Value getnetworkhashps(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "getnetworkhashps\n"
            "Returns an estimate of the HanaCoin network hash rate.");
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "HanaCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "HanaCoin is downloading blocks...");
